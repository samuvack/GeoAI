...
# Synoniemwoordenboek (indien nodig aan te vullen)
synonym_dict = {
...
# Haal de vragen en SQL-query's op
indexed_questions = [item['vraag'] for item in training_data]
sql_queries = [item['query'] for item in training_data]
...
with open('sql_queries.json', 'w', encoding='utf-8') as f:
    json.dump(sql_queries, f, ensure_ascii=False, indent=2)
